...
function createTrailMaterial(atlas, palette, lightDir) {
  ...
  return new THREE.ShaderMaterial({
    ...
    vertexShader: `
      ...
      void main() {
        vec3 p = position;
        p.x += sin(uTime * (2.4 + fract(aSeed * 0.7)) + aSeed * 34.0) * 0.004;
        p.y += cos(uTime * (2.8 + fract(aSeed * 0.8)) + aSeed * 41.0) * 0.004;
        vec4 mvPosition = modelViewMatrix * vec4(p, 1.0);
        float life = clamp(aLife, 0.0, 1.0);
        gl_Position = projectionMatrix * mvPosition;
        gl_PointSize = (22.0 * aSize * life) / max(0.6, -mvPosition.z);
        vDigit = mod(floor(uTime * (3.0 + fract(aSeed * 2.0)) + aSeed * 19.0), 2.0);
        vPalette = fract(aSeed * 9.3 + uTime * 0.03);
        vAlpha = life;
      }
    `,
    fragmentShader: `
      ...
      void main() {
        ...
        float radial = smoothstep(0.95, 0.18, distance(gl_PointCoord, vec2(0.5)));
        float alpha = glyph.a * radial * vAlpha * uAlpha;
        if (alpha <= 0.01) discard;
        vec3 color = samplePalette(vPalette);
...
export class MothSystem {
  constructor(options) {
    ...
    this.trailEmitterBounds = {
      center: new THREE.Vector3(),
      size: new THREE.Vector3(0.06, 0.06, 0.06)
    };
    this.trailEmitClock = 0;
    this.ghosts = [];
  ...
  fitMothScale() {
    ...
    this.trailEmitterBounds.center.set(0, 0, 0);
    this.trailEmitterBounds.size.set(
      Math.max(0.028, mothSize.x * scale * 0.40),
      Math.max(0.02, mothSize.y * scale * 0.24),
      Math.max(0.028, mothSize.z * scale * 0.46)
    );
  }
  ...
  onActionFinished() {
    if (this.pendingActionKey) {
      ...
      if (next === "perch") {
        ...
        if (next === "fly" || next === "flySad") this.perched = false;
        this.playLoop(next);
      }
      return;
    }
  ...
  initTrailSystem() {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      this.trail.push({
        ...
        life: 0,
        maxLife: this.cfg.trailLife * (0.8 + Math.random() * 0.45),
        size: sizes[i],
        seed: seeds[i]
      });
    }
    ...
    this.trailGeometry.setAttribute("aSeed", new THREE.BufferAttribute(seeds, 1));
    this.trailGeometry.setAttribute("aLife", new THREE.BufferAttribute(lives, 1).setUsage(THREE.DynamicDrawUsage));

    this.trailMaterial = createTrailMaterial(this.glyphAtlas, this.palette, this.lightDir);
  ...
  emitTrailParticle() {
    ...
    let particle = this.trail.find((item) => !item.alive);
    if (!particle) {
      particle = this.trail.reduce((oldest, current) => (current.life < oldest.life ? current : oldest), this.trail[0]);
    }

    const idx = this.trail.indexOf(particle);
    const local = this.temp.d.set(
      (Math.random() - 0.5) * this.trailEmitterBounds.size.x,
      (Math.random() - 0.2) * this.trailEmitterBounds.size.y,
      (Math.random() - 0.5) * this.trailEmitterBounds.size.z
    ).add(this.trailEmitterBounds.center);
    const world = this.root.localToWorld(local.clone());
    const drift = this.velocity.clone().multiplyScalar(-this.cfg.trailSpeedFactor)
      .add(new THREE.Vector3(
        (Math.random() - 0.5) * this.cfg.trailVelocityJitter,
        (Math.random() - 0.5) * this.cfg.trailVelocityJitter * 0.65,
        (Math.random() - 0.5) * this.cfg.trailVelocityJitter
      ));

    particle.alive = true;
    particle.position.copy(world);
    particle.velocity.copy(drift);
    particle.life = particle.maxLife;
    particle.maxLife = this.cfg.trailLife * (0.82 + Math.random() * 0.42);
    particle.size = 0.7 + Math.random() * 0.85;

    const positions = this.trailGeometry.attributes.position.array;
    const sizes = this.trailGeometry.attributes.aSize.array;
    const lives = this.trailGeometry.attributes.aLife.array;
    const ptr = idx * 3;
    positions[ptr] = world.x;
    positions[ptr + 1] = world.y;
    positions[ptr + 2] = world.z;
    sizes[idx] = particle.size * this.cfg.trailPointScale;
    lives[idx] = 1;
  }

  updateTrail(elapsed, delta) {
    if (!this.trailGeometry || !this.trailMaterial || !this.trailPoints) return;

    const speed = this.velocity.length();
    const emitStrength = this.perched ? 0.18 : THREE.MathUtils.clamp(speed / Math.max(0.001, this.cfg.flySpeed * 0.95), 0, 1.6);
    this.trailEmitClock += delta * (0.6 + emitStrength * 2.2 + (this.voidState?.active ? 1.0 : 0));

    while (this.trailEmitClock >= this.cfg.trailEmitInterval) {
      this.trailEmitClock -= this.cfg.trailEmitInterval;
      if (emitStrength > 0.08 || this.voidState?.active) {
    ...
    for (let i = 0; i < this.trail.length; i += 1) {
      ...
      if (!particle.alive) {
        positions[ptr] = 9999;
        positions[ptr + 1] = 9999;
        positions[ptr + 2] = 9999;
        lives[i] = 0;
        continue;
      }
      ...
      particle.velocity.multiplyScalar(Math.max(0, 1 - this.cfg.trailDrag * delta));
      particle.velocity.y += delta * 0.018;
      particle.position.addScaledVector(particle.velocity, delta);

      positions[ptr] = particle.position.x;
      positions[ptr + 1] = particle.position.y;
      positions[ptr + 2] = particle.position.z;
      lives[i] = THREE.MathUtils.clamp(particle.life / particle.maxLife, 0, 1) * this.cfg.trailAlpha;
    }

    this.trailGeometry.attributes.position.needsUpdate = true;
    this.trailGeometry.attributes.aLife.needsUpdate = true;
    this.trailGeometry.attributes.aSize.needsUpdate = true;
    this.trailMaterial.uniforms.uTime.value = elapsed;
    this.trailMaterial.uniforms.uAlpha.value = this.visible ? 1 : 0;
  ...
  setVisibility(visible) {
    ...
    if (this.trailPoints) this.trailPoints.visible = this.visible;
  ...
  update(context) {
    ...
    this.updateTrail(elapsed, delta);
