...
export class MothSystem {
  ...
  update(context) {
    ...
    this.resolveStuckState(elapsed, hoveredIndex, coverWorldData);
  ...
  updateStateAndMotion(delta, elapsed, hoveredEntry, hoveredIndex, coverWorldData) {
    ...
    if (hasVoid && shouldAvoidVoid) {
      ...
      if ((this.mode === "landed" || this.mode === "landing") && this.currentPerchTarget?.type !== "home") {
        this.startTakeoff(elapsed);
      } else if (this.mode !== "takeoff"  && this.mode !== "inspectVoid") {
      ...
      if (!hasVoid && this.mode !== "takeoff"  && this.mode !== "inspectVoid" && this.mode !== "restHome") {
        this.mode = "patrol";
      }
    ...
    if (this.mode === "approachHome" && homeTarget) {
      ...
      if (distance <= (this.cfg.homeApproachDistance || this.cfg.landTriggerDistance || 0.18)) {
        this.currentPerchTarget = { type: "home" };
        this.mode = "landing";
        this.perched = false;
        if (!this.playOnce("land", "perch")) {
          this.forcePerch({ type: "home", ...homeTarget });
          return;
        }
      }
    ...
    if (this.mode === "approachCover" && coverTarget) {
      ...
      if (distance <= (this.cfg.landTriggerDistance || 0.18)) {
        this.currentPerchTarget = { type: "cover", coverIndex: hoveredIndex };
        this.mode = "landing";
        this.perched = false;
        if (!this.playOnce("land", "perch")) {
          this.forcePerch({ type: "cover", coverIndex: hoveredIndex, ...coverTarget });
  ...
  startTakeoff(elapsed) {
    ...
    this.actionLockedUntil = 0;
      ...
      this.mode = this.voidState?.active ? "approachVoid" : "patrol";
      this.playLoop(this.voidState?.active ? "fly" : this.getPatrolFlightAction());
  ...
  forcePerch(targetInfo) {
    if (!targetInfo?.position) return false;

    const perchType = targetInfo.type === "home" ? "home" : "cover";
    this.clearCursorAttraction();
    this.currentPerchTarget = perchType === "home"
      ? { type: "home" }
      : { type: "cover", coverIndex: typeof targetInfo.coverIndex === "number" ? targetInfo.coverIndex : null };
    ...
    const lookPoint = targetInfo.position.clone().add(targetInfo.normal || new THREE.Vector3(0, 0, 1));
    this.lookAtPoint(lookPoint, targetInfo.up || new THREE.Vector3(0, 1, 0), 0.42);
    this.playLoop("perch");
    this.lastProgressAt = this.getElapsed();
    return true;
  }

  resolveStuckState(elapsed, hoveredIndex, coverWorldData) {
    const mode = this.mode;
    const settled = mode === "restHome" || mode === "landed" || mode === "inspectVoid" || mode === "cursorInteract";
    if (settled) {
      this.lastProgressAt = elapsed;
      return;
    }

    const movementDelta = this.root.position.distanceTo(this.temp.center);
    const speed = this.velocity.length();

    let target = null;
    const coverIndex = typeof hoveredIndex === "number" && hoveredIndex >= 0
      ? hoveredIndex
      : (this.currentPerchTarget?.type === "cover" ? this.currentPerchTarget.coverIndex : null);

    if (mode === "approachHome") {
      target = this.getHomePerchTarget()?.position || null;
    } else if (mode === "approachCover" || mode === "landing") {
      if (typeof coverIndex === "number") {
        target = this.getCoverPerchTarget(coverIndex, coverWorldData)?.position || null;
      }
    } else if (mode === "approachVoid") {
      target = this.getVoidInspectTarget()?.position || null;
    } else if (mode === "companion") {
      target = this.getCompanionTarget();
    } else if (mode === "cursorCurious") {
      target = this.getCursorAttractionTarget();
    } else if (mode === "flee") {
      target = this.getFleeTarget();
    } else if (mode === "patrol" || mode === "takeoff") {
      target = this.currentPatrolAnchor;
    }

    const thresholdSpeed = this.cfg.stuckSpeedThreshold || 0.045;
    const thresholdDistance = this.cfg.stuckDistanceThreshold || 0.24;
    const timeout = this.cfg.stuckTimeout || 1.05;

    if (!target) {
      if (movementDelta > 0.0025 || speed > thresholdSpeed) this.lastProgressAt = elapsed;
      return;
    }

    const distanceToTarget = this.root.position.distanceTo(target);
    if (movementDelta > 0.0025 || speed > thresholdSpeed || distanceToTarget <= thresholdDistance) {
      this.lastProgressAt = elapsed;
      return;
    }

    if (elapsed - this.lastProgressAt < timeout) return;
    ...
    if ((mode === "approachCover" || mode === "landing") && typeof coverIndex === "number") {
      const perchTarget = this.getCoverPerchTarget(coverIndex, coverWorldData);
      if (perchTarget) {
        this.forcePerch({ type: "cover", coverIndex, ...perchTarget });
        return;
      }
    }
